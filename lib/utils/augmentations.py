...
def random_perspective_for_img(combination, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    """combination of img transform"""
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    targets_0, targets_1, targets_2 = targets[0], targets[1], targets[2]
    img = combination
    height = img.shape[0] + border[0] * 2  # shape(h,w,c)
    width = img.shape[1] + border[1] * 2
    ...
    if n != 0:
        # warp points
        xy = np.ones((n * 4, 3))
        xy[:, :2] = targets_0[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
        xy = xy @ M.T  # transform
        if perspective:
            xy = (xy[:, :2] / xy[:, 2:3]).reshape(n, 8)  # rescale
        else:  # affine
        ...
        # filter candidates
        i = _box_candidates(box1=targets_0[:, 1:5].T * s, box2=xy.T)
        targets_0 = targets_0[i]
        targets_0[:, 1:5] = xy[i]
    ...
    if n:
        # warp points
        xy = np.ones((n * 4, 3))
        xy[:, :2] = targets_1[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
        xy = xy @ M.T  # transform
        if perspective:
            xy = (xy[:, :2] / xy[:, 2:3]).reshape(n, 8)  # rescale
        else:  # affine
        ...
        # filter candidates
        i = _box_candidates(box1=targets_1[:, 1:5].T * s, box2=xy.T)
        targets_1 = targets_1[i]
        targets_1[:, 1:5] = xy[i]
    ...
    if n:
        # warp points
        xy = np.ones((n * 4, 3))
        xy[:, :2] = targets_2[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
        xy = xy @ M.T  # transform
        if perspective:
            xy = (xy[:, :2] / xy[:, 2:3]).reshape(n, 8)  # rescale
        else:  # affine
        ...
        # filter candidates
        i = _box_candidates(box1=targets_2[:, 1:5].T * s, box2=xy.T)
        targets_2 = targets_2[i]
        targets_2[:, 1:5] = xy[i]

    combination = img
    return combination, [targets_0, targets_1, targets_2]
