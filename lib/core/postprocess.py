...
def build_targets_multi(cfg, predictions, targets, model, head_index):
    '''
    predictions
    [16, 3, 32, 32, 85]
    [16, 3, 16, 16, 85]
    ...
    # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
    if head_index == 0:
        det = model.module.model[model.module.det_out_idx] if is_parallel(model) \
            else model.model[model.det_out_idx]  # Detect() 
    elif head_index == 1:
        det = model.module.model[model.module.det_out_idx_1] if is_parallel(model) \
            else model.model[model.det_out_idx_1]  # Detect()
    else:
        det = model.module.model[model.module.det_out_idx_2] if is_parallel(model) \
            else model.model[model.det_out_idx_2]
